...
def title():
    waktu = strftime('%H:%M')
    tanggal = str(cal.selection_get())
    root.title(tanggal + " | " + waktu + " | Calender Ahmad Naufal Diwantara Putra")
...
cal = Calendar(root, font='Times 14 bold italic', selectmode='day', locale='id_ID', cursor='arrow', background = "#008F7A" , disabledbackground = "red" , borderbackground = "white" , headersbackground = "#FFC75F" , foreground='black', selectbackground='#32CCBC')
...
btnAdd = tk.Button(root, text="Tambah", width=20, bg="lightblue", font='Times 10', cursor='hand2', command=AddForm)
btnAdd.grid(row=4, column=1, sticky="N")

btnDel = tk.Button(root, text="Hapus", width=20, bg="lightblue", font='Times 10', cursor='hand2', fg="red", command=delTodo)
btnDel.grid(row=4, column=2, sticky="N")

btnLoad = tk.Button(root, text="Load", width=20, bg="lightblue", font='Times 10', cursor='hand2', command=LoadTodos)
btnLoad.grid(row=6, column=1, sticky="S")

btnSave = tk.Button(root, text="Save", width=20, bg="lightblue", font='Times 10', cursor='hand2', command=SaveTodos)
